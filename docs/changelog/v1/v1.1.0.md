...
Specification & assurance hardening (Milestone 11): the post-1.0 specification review closed out
across seven PRs. Documentation- and assurance-focused, with one new (opt-in) build surface for
security fuzzing. No breaking changes; the 1.0 public API is unchanged.
...
- See the `Added` **Security** entry: the untrusted-input components are now continuously exercised
  by libFuzzer harnesses under ASan/UBSan, and the hash functions' non-cryptographic scope is
  documented (ADR-0031).
